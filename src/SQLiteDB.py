...
class SQLiteDB(EliteDB.EliteDB):
  ...
  def queryProfitWindow(self,x,y,z,windowsize=30,maxdistance=30,minprofit=1000,landingPadSize=0):
    ...
    WITH systemwindow AS (
      SELECT systems.name AS systemname, bases.name AS basename, commodityPrices.baseId AS baseId, systemId, commodityId, exportPrice, supply, importPrice, demand, commodities.name AS commodityname, average, x, y, z FROM
        commodities,
        commodityPrices,
        bases,
        baseInfo,
    ...
    if not landingPadSize>0: # query without landingpads
      querystring="""
      WITH systemwindow AS (
        SELECT systems.name AS systemname, bases.name AS basename, commodityPrices.baseId, systemId, commodityId, exportPrice, supply, importPrice, demand, commodities.name AS commodityname, average, x, y, z FROM
          commodities,
          commodityPrices,
          bases,
          systems
